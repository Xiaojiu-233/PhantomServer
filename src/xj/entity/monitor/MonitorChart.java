...
public class MonitorChart {
    ...
    private List<Integer[]> list;// 数据数组
    ...
    private void refreshInputData(){
        ...
        if(timeDelta >= MONITOR_CHART_UNIT_TIME * 1000L){
            if(isInput){
                ...
                list.add(new Integer[]{timeNum,storeNumber});
                storeNumber = 0;
                isInput = false;
            }
    ...
    public Map<String,Object> outputChart(int k) {
        ...
        // 分析数据
        int[] yRet = new int[MONITOR_CHART_X_RANGE + 1];
        ...
        for(int i=1;i<xRange.length;i++){
            ...
            x.add(minusTime.getMinute() + StrPool.COLON + minusTime.getSecond());
            // y轴
            if (isAccel){
                y.add(yRet[i]);
            } else {
